...
/// Version ID for Pokémon Brilliant Diamond, as stored in the PB8 `Version`
/// byte at `0x0DE` (PKHeX `GameVersion.BD`).
const VERSION_BRILLIANT_DIAMOND: u8 = 48;
/// Sentinel met/egg location used by BDSP for "no location" (`0xFFFF`).
const LOCATION_NONE: u16 = 0xFFFF;

#[derive(Debug, Clone, Copy, Default)]
pub struct BdspFactory;

impl PokemonFactory for BdspFactory {
    type Output = BdspPokemon;
    ...
    fn gen_pokemon_from_species(
        ...
        ot_name: &str,
        ot_id: TrainerID,
    ) -> Result<BdspPokemon, PokemonError> {
        let dex_num = BdspGameData
            .nat_dex_num(species)
            .map_err(|_| PokemonError::UnknownSpecies(species.to_string()))?;

        let mut new_pokemon = BdspPokemon::new();
        ...
        let level = BdspGameData.lowest_level(dex_num).max(1);
        ...
        // Egg/Met locations default to the BDSP "none" sentinel (0xFFFF).
        ...
        let (ability1_id, _, _) = BdspGameData.ability_ids(dex_num).unwrap_or((0, None, None));
