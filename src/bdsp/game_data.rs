...
pub struct BdspGameData;

impl GameData for BdspGameData {
    type Result<R> = rusqlite::Result<R>;

    fn held_items(&self) -> Result<Vec<String>> {
        let conn = Connection::open("pk_edit.db")?;
        let mut stmt =
            conn.prepare("SELECT name_en FROM items WHERE game_family = 'bdsp' AND holdable = 1")?;
    ...
    fn nat_dex_num(&self, species: &str) -> Result<u16> {
        ...
        let res = conn.query_row(
            "SELECT dex_num FROM species WHERE name_en LIKE ?1 AND game_family = 'bdsp'",
            [species],
            |row| row.get(0),
        );
        let _ = conn.close();
    ...
    fn growth_rate(&self, dex_num: u16) -> Result<String> {
        let conn = Connection::open("pk_edit.db")?;
        let res = conn.query_row(
            "SELECT growth_rate FROM species WHERE dex_num = ?1 AND form = 0 AND game_family = 'bdsp'",
            [dex_num],
            |row| row.get(0),
        );
        let _ = conn.close();
    ...
    fn pk_species(&self, dex_num: u16) -> Result<String> {
        let conn = Connection::open("pk_edit.db")?;
        let res = conn.query_row(
            "SELECT name_en FROM species WHERE dex_num = ?1 AND form = 0 AND game_family = 'bdsp'",
            [dex_num],
            |row| row.get(0),
        );
        let _ = conn.close();
    ...
    fn move_data(&self, id: usize) -> Result<(String, String, u8)> {
        ...
        let res = conn.query_row(
            "SELECT COALESCE(t.name_en, 'Normal'), m.name_en, m.pp
             FROM moves m
             LEFT JOIN types t ON m.type = t.id AND t.game_family = 'bdsp'
             WHERE m.id_in_game = ?1 AND m.game_family = 'bdsp'",
            [id],
    ...
    fn typing_form(&self, dex_num: u16, form: u8) -> Result<(String, Option<String>)> {
        ...
        let res = conn.query_row(
            ...
            FROM species s
            JOIN types t1 ON s.type1 = t1.id AND t1.game_family = 'bdsp'
            LEFT JOIN types t2 ON s.type2 = t2.id AND t2.game_family = 'bdsp'
            WHERE s.dex_num = ?1 AND s.form = ?2 AND s.game_family = 'bdsp'",
    ...
    fn gender_ratio(&self, dex_num: u16) -> Result<u8> {
        ...
        let ratio: u8 = conn.query_row(
            "SELECT gender_ratio FROM species WHERE dex_num = ?1 AND form = 0 AND game_family = 'bdsp'",
            [dex_num],
            |row| row.get(0),
        )?;
        let _ = conn.close();
    ...
    fn abilities_form(&self, dex_num: u16, form: u8) -> Self::Result<Abilities> {
        ...
        let res = conn.query_row(
            "SELECT a1.name_en, a2.name_en, ah.name_en
             FROM species s
             JOIN abilities a1 ON s.ability1 = a1.id_in_game AND a1.game_family = 'bdsp'
             LEFT JOIN abilities a2 ON s.ability2 = a2.id_in_game AND a2.game_family = 'bdsp'
             LEFT JOIN abilities ah ON s.hidden_ability = ah.id_in_game AND ah.game_family = 'bdsp'
             WHERE s.dex_num = ?1 AND s.form = ?2 AND s.game_family = 'bdsp'",
            rusqlite::params![dex_num, form],
            |row| {
                Ok(Abilities {
    ...
    fn ability_ids_form(
        ...
        let res = conn.query_row(
            "SELECT ability1, ability2, hidden_ability FROM species
             WHERE dex_num = ?1 AND form = ?2 AND game_family = 'bdsp'",
            rusqlite::params![dex_num, form],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        );
        let _ = conn.close();
    ...
    fn species(&self) -> Result<Vec<String>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT name_en FROM species WHERE game_family = 'bdsp' AND form = 0 ORDER BY dex_num",
        )?;
        let rows = stmt.query_map([], |row| row.get(0))?;
        let mut res = Vec::new();
        for result in rows {
    ...
    fn moves(&self) -> Result<Vec<String>> {
        let conn = Connection::open("pk_edit.db")?;
        let mut stmt = conn
            .prepare("SELECT name_en FROM moves WHERE game_family = 'bdsp' ORDER BY id_in_game")?;
        let rows = stmt.query_map([], |row| row.get(0))?;
        let mut res = Vec::new();
        for result in rows {
            res.push(result?);
    ...
    fn evolution(&self, dex_num: &u16) -> Self::Result<Evolution> {
        let conn = Connection::open("pk_edit.db")?;
        let res = conn.query_row(
            "SELECT evolves_into, method, condition FROM evolutions WHERE dex_num = ?1 AND game_family = 'bdsp'",
            [dex_num],
            |row| {
                Ok(Evolution {
    ...
    fn items_in_pocket(&self, pocket: Pocket) -> Self::Result<Vec<String>> {
        ...
        let conn = Connection::open("pk_edit.db")?;
        let mut stmt = conn.prepare(
            "SELECT name_en FROM items WHERE game_family = 'bdsp' AND pocket = ?1 ORDER BY id_in_game",
        )?;
    ...
    fn item_flavor_text(&self, name: &str) -> String {
        item_flavor_text_for("bdsp", name)
    }

    fn balls_id(&self) -> Result<Vec<u16>> {
        let conn = Connection::open("pk_edit.db")?;
        let mut stmt = conn.prepare(
            "SELECT id_in_game FROM items WHERE game_family = 'bdsp' AND pocket = 'balls'",
        )?;
        let rows = stmt.query_map([], |row| row.get(0))?;
        let mut res = Vec::new();
        for result in rows {
    ...
    fn item_sprite_id(&self, name: &str) -> Result<usize> {
        let conn = Connection::open("pk_edit.db")?;
        let res = conn.query_row(
            "SELECT sprite_id FROM items WHERE name_en = ?1 AND game_family = 'bdsp'",
            [name],
            |row| row.get(0),
        );
        let _ = conn.close();
    ...
    fn balls_sprite_ids(&self) -> Result<Vec<u16>> {
        let conn = Connection::open("pk_edit.db")?;
        let mut stmt = conn.prepare(
            "SELECT sprite_id FROM items WHERE game_family = 'bdsp' AND pocket = 'balls' AND sprite_id IS NOT NULL",
        )?;
        let rows = stmt.query_map([], |row| row.get(0))?;
        let mut res = Vec::new();
        for result in rows {
    ...
    fn lowest_level(&self, dex_num: u16) -> u8 {
        ...
        let res = conn.query_row(
            "SELECT condition FROM evolutions WHERE evolves_into = ?1 AND game_family = 'bdsp' AND method = 'Level'",
            [dex_num],
