...
use crate::lumi::pokemon::data::Stats;
...
impl LumiPokemon {
    ...
    pub fn init_stats(&mut self) {
        ...
        let form = self.form();
        let (base_hp, base_atk, base_def, base_spa, base_spd, base_spe) = LumiGameData
...
impl Pokemon for LumiPokemon {
    ...
    fn species(&self) -> String {
        ...
        LumiGameData.pk_species(dex).unwrap_or_default()
    }
    ...
    fn held_item(&self) -> Option<String> {
        ...
        Connection::open("pk_edit.db").ok().and_then(|conn| {
            conn.query_row(
                "SELECT name_en FROM items WHERE id_in_game = ?1 AND game_family = 'lumi'",
                [id as usize],
                |row| row.get(0),
            )
            .ok()
    ...
    fn moves(&self) -> Vec<Move> {
        ...
        for i in 0..4usize {
            ...
            if let Ok((move_type, name, max_pp)) = LumiGameData.move_data(move_id as usize) {
    ...
    fn set_move(&mut self, slot: usize, attack: &str) -> Result<(), PokemonError> {
        ...
        let (id, pp) = Connection::open("pk_edit.db")
            .and_then(|conn| {
                conn.query_row(
                    "SELECT id_in_game, pp FROM moves WHERE name_en = ?1 AND game_family = 'lumi'",
                    [attack],
    ...
    fn ability(&self) -> String {
        match LumiGameData.abilities_form(self.nat_dex_number(), self.form()) {
            Ok(abilities) => {
...
impl LumiPokemon {
    fn update_exp_for_level(&mut self, level: u8) {
        let dex = self.nat_dex_number();
        let growth = LumiGameData.growth_rate(dex).unwrap_or_default();
